...
class Configuration(commands.Cog, name="Bot Configuration Command"):
  ...
  async def send_forum_form(self,
                           ctx: commands.Context,
                           forum_channel: nextcord.ForumChannel = None):
    """Отправляет форму в форум, которая будет автоматически создавать публикации"""
    
    # Если форум не указан, пытаемся использовать настроенный
    if not forum_channel:
      try:
        ...
        if os.path.exists(config_file):
          with open(config_file, 'r', encoding='utf-8') as f:
            ...
            if config.get("forum_channel_id"):
              ...
              if not forum_channel:
                await ctx.send("❌ Настроенный форум не найден! Сначала настройте форум командой `!setupforum`")
                return
            else:
        ...
        await ctx.send(f"❌ Ошибка загрузки конфигурации: {str(e)}")
        return
